...
function renderSliderCovers(albums) {
	...
	for (let i = 0; i < 10; i++) {
		...
		if (albumData && itemDiv) {
			// HTML 구조가 이미 제목/아티스트를 포함하도록 변경되었으므로,
			// itemDiv 내부 요소만 업데이트합니다.
			...
			if (img) {
				img.src = albumData.album_cover_url;
				img.alt = `${albumData.track_name} - ${albumData.artist} 앨범 커버`;
				// 스타일은 HTML 또는 CSS에 정의되어 있다고 가정하고, 여기서 인라인 스타일을 강제하지 않습니다.
				// 다만, 이전 코드에서처럼 style 속성을 강제로 덮어씌우는 대신,
				// HTML에 정의된 클래스(img-fluid, rounded-3)와 인라인 스타일을 활용하도록 합니다.
				img.style.opacity = '0.9';
			...
			// 💡 추가: 링크 설정 (클릭 시 YouTube로 이동하도록 전체 항목에 이벤트 추가)
			itemDiv.style.cursor = 'pointer'; // 클릭 가능함을 표시
			itemDiv.onclick = () => {
	...
	for (let i = 0; i < 10; i++) {
		...
		if (clonedItemDiv) {
			...
			// 복제본에도 클릭 이벤트가 제대로 작동하도록 복사된 HTML을 재파싱한 후 이벤트를 다시 설정해주는 것이 안전합니다.
			// 하지만 간단하게 처리하기 위해, HTML 복사 후, 원본의 클릭 이벤트를 복사된 요소에 다시 설정해 줍니다.
			const albumData = albums[i];
